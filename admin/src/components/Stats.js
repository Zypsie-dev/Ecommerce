    ...
    <Container>
      <LStatsContainer>
        <LStats color="0694A2">
          <TodayIcon />
          <p>Today's Orders</p>
          <p>{orderPrice[0]?.today ?? "N/A"}</p>
        </LStats>
        <LStats color="3F83F8">
          <LocalShippingIcon />
          <p>Monthly Orders</p>
          <p>{orderPrice[0]?.month ?? "N/A"}</p>
        </LStats>
        <LStats color="0E9F6E">
          <AccountBalanceIcon />
          <p>Total Orders</p>
          <p>{orderPrice[0]?.allTime ?? "N/A"}</p>
        </LStats>
      </LStatsContainer>
      ...
      <OrdersStatsContainer>
        <OrderStats backColor="FEECDC" logoColor="D03801">
          ...
          <div>
            <p>Total</p>
            <p>
              {(orderStats[0]?.pending ?? 0) +
                (orderStats[0]?.processing ?? 0) +
                (orderStats[0]?.delivered ?? 0)}
            </p>
          </div>
        </OrderStats>
        ...
        <OrderStats backColor="D5F5F6" logoColor="117C88">
          ...
          <div>
            ...
            <p>{orderStats[0]?.processing ?? "N/A"}</p>
          </div>
        </OrderStats>
        <OrderStats backColor="DEF7EC" logoColor="057A55">
          ...
          <div>
            <p>Order Delivered</p>
            <p>{orderStats[0]?.delivered ?? "N/A"}</p>
          </div>
        </OrderStats>
      </OrdersStatsContainer>
    </Container>
